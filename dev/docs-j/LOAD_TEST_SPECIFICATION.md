...
#### CP01/CP02 の一次情報はサーバの audit ログ

2026-08-13 以降、判定はサーバ自身の audit ログ（plugin 側 `[B6]`）を一次情報とし、client console
は副次とする。両方の件数をレポートに載せ、どちらを採ったかを明記する。

client 区間は真の保持区間の**部分集合**である。ACQUIRED は取得後・RELEASED は解放前に、いずれも
lock ブロックの内側で出力されるため。したがって **client で見えた重なりは本物**だが、**本物が
client で見えないことはあり得る**。audit ログは資源の持ち手が変わる瞬間をサーバ側で記録するので、
この非対称が無い。

判定元を変更したのは、下記の未解明な観測を追っている最中である。**歓迎しない数字を見た後に判定元を
変えた**という事実は、それ自体が判断の偏りになり得るので記録しておく。変更の理由は上記の非対称性で
あって観測値ではないが、順序が逆であったなら疑うべき変更だった。

#### 未解明: 2026-08-12 stress の重なり 10 件

plugin `29c05d3` の stress 実行で、client 判定による重なりが 10 件出た。以降 6 回の実行（うち 2 回は
競合を狙った `timeout-race`）で再現していない。**未解明のまま残す**。生データは「各種別の最新のみ
保持」の慣例に従い破棄済みなので、再発時はその時点の物証で改めて調査する。

切り分けで排除できたもの:

| 仮説 | 判定 | 根拠 |
|---|---|---|
| 解析の欠陥（誤ペアリング） | 排除 | 10 組 20 側すべて健全。同一 `(uid,iter,phase,event)` の重複 0、生イベントと start/end が一致、共有資源が両者の `resources` に実在、target 一致 |
| 報告側のバグ（client が別の資源名を信じた） | 排除 | 別実行の REMOTE_MAIN 565 件で、client の資源集合とサーバ付与集合が完全一致（不一致 0）。設計上も単一の `resources` リストが audit と lockEnvVars の両方を作る |
| ハーネスの前提（emit 位置） | 排除 | `Jenkinsfile.grid` の emit はコード上 lock ブロックの内側 |
| 付与経路の非同期 | 排除 | 付与 5 経路（ローカル即時 / ローカル昇格 / リモート即時 / リモート昇格 / freestyle）すべてが単一モニタ `syncResources` の内側。A6 が追加したタイマーも同モニタを取ってから昇格する |

観測と設計が両立しない状態で残っている。どちらかの前提にまだ穴がある。
