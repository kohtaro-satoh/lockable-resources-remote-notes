...
| 日付 | 内容 |
|---|---|
| 2026-08-15 | **jacoco コンテナ計測基盤を整備し、3 層の到達率を初めて実測**（`dev/jenkins-env/coverage/`）。既定は無効（計装は load が測っている時間そのものを歪めるため）、`--coverage` で有効化。`includes` をプラグインのパッケージに限定（付けないと Jenkins core 全体が計装され exec が肥大しレポートが埋もれる）。**実測: ユニット 88.4%/72.5%、E2E 82.7%/63.6%、load 54.4%/34.4%、3 層の和 93.2%/80.8%**（行/分岐、remote 関連）。和がユニット単独を上回る＝**E2E と load が独自に踏む領域が実在**する。分岐は 72.5%→80.8% と差が大きく、実環境でしか通らない分岐が相当ある。**未到達の remote クラスはゼロ**、残余は 88 行/105 分岐で最大は `RemoteLockSession`（3 層すべてで最下位）。`RemoteCallback` と `RemoteApiException` は行が通っているのに**分岐 0.0%**。**構築中に計測基盤自身のバグを 2 件発見**: ①シナリオがコントローラを再作成すると compose が環境を組み直すためエージェントが脱落し、以降そのコントローラが**無言で記録されなくなる**（スイートは通るので気付けない。`lib/coverage-env.sh` に定義を切り出し `common.sh` 経由で継承）②エージェントは JVM シャットダウンフックで書くため docker 既定の 10 秒 SIGTERM 猶予では取りこぼす（`stop_grace_period: 120s`）。**発見の端緒は「S19 が PASS しているのに `ResourcesResource` が 0%」という矛盾**で、最初の数字（69.8%）を受け入れていたら 13 ポイント低い誤値を成果として報告していた |
