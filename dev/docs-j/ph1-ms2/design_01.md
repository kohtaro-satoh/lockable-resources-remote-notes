...
「全コントローラが 60s ごとに相互の `/resources` を引く」負荷を明示的に足すかは **実装後に判断**する
...
| Q5 | `/resources` を負荷テストに含めるか | **実装後に判断。** ただし条件が付いた: **LR ページが正式な挙動で更新されている状態のまま負荷試験を回せること**。つまり負荷時にクライアント側リモートビューを無効化・迂回しない。`/resources` の短期キャッシュ（60s TTL）が実運用どおり効いた状態で計測する |
...
| Q8 | issue 本文の更新をいつ出すか | **(b) 先に本文だけ直してから実装。更新作業はユーザーが実施**し、こちらは修正内容を並走で助言する |
