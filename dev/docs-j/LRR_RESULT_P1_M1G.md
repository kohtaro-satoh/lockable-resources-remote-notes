...
## 検証（PR ブランチ `feature/1025-remote-lr-p1-m1` `4f3577f` で最終実施）

下記レポートは **PR ブランチ `feature/1025-remote-lr-p1-m1`（現 upstream master `87c4a7e` 上の単一 squash コミット `4f3577f`、
コメント整理込み）** での値。squash 前（`808da92`）とツリー同一で、リベース版（`4b40a42`）・元 m1g（`57d2e6d`）でも同値を確認済み。

- **mvn フル: 382 件 / 0 失敗 / 1 skip / BUILD SUCCESS**（`dev/reports/20260615141539-mvn-test.log`、worktree、
  コミット済み HEAD `4f3577f`）。**テスト不変・挙動保存の裏付け**（新規ユニットは追加せず、既存が回帰網。
  `LockStepWithRestartTest` 通過＝リベースの Serializable 解決の妥当性も実証）。
- **E2E `--clean-start`: 20/20 PASS / fail 0**（`dev/reports/20260615143511-e2e-test.md`）。移動コードを実環境で踏む
...
## PR ブランチ整形（squash＋コメント整理、2026-06-15）

PR レビューを経て、提出ブランチを `feature/1025-remote-lr-p1-m1` に確定し整形した。
- **コメント整理（コメントのみ・挙動不変）**: remote 追加コードのコメントを ASCII 限定化（em dash/→/×/… 変換、upstream コメントは不変）、
  Phase/Milestone・判断変遷の記述（M1B–M1G/H-1/L-b,c,d/「extracted in」等）を削除し「今の仕様」のみに、phase 1 積み残しは
  `issue #1025 phase 1` 表記で残置（`heartbeatIntervalSeconds` は受理するが無視＝`RemoteApiV1Action`）。
- **squash**: `10d3d48`(squash M1A–M1F)＋`4b40a42`(M1G)＋`808da92`(コメント整理) を master `87c4a7e` 上の
  **単一コミット `4f3577f`「Remote Lockable Resources (issue #1025 phase 1)」** に集約。全機能差分 47 ファイル / +5594・-44。
- 元 `4b40a42`(`-m1g-rebased`)・`57d2e6d`(`-m1g`) は参照として保存（可逆）。

## コミット

- plugin: **PR ブランチ `feature/1025-remote-lr-p1-m1` = `4f3577f`**（単一コミット）。push なし（提出時に force push）。
- notes は本ステップでコミット（DESIGN/STEPS/RESULT j+e、README Status/ブランチ、レポート最新ひとつずつ整理）。
