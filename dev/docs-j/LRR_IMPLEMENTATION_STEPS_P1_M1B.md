...
- `run-e2e.sh --clean-start`（HEAD `ba670e7` の HPI で 4 コンテナ新規起動）: **11/12 PASS**
...
- [x] コミット済み（notes リポジトリ `1ef27e7`）

記録: 2026-06-12 完了。S08 シナリオ修正は `4f5ccab`、S10〜S13 追加とレポートは `1ef27e7`。

---

### 追補（Follow-up）: M1B 残課題 3 件の回収

**目的:**
Step 8 完了後に残っていた 3 件（Step 1-d 繰り越し、レビュー 4-4、レビュー 5-1）を
M1B 追加作業として回収する。

#### F-1. forcedServerId 保存時バリデーション（Step 1-d 回収、ドリフト #4）

- `LockableResourcesManager.doCheckForcedServerId()`: remotes に存在しない値に
  FormValidation.warning を返すフォームバリデーション
- `configure()` にバインド完了後の整合チェック + 警告ログを追加
  （フォームのフィールド順で forcedServerId が remotes より先にバインドされるため、
  setter 時チェックは不正確になる — バインド後チェックが正解）
- `Messages.properties` に `warning.forcedServerIdNotConfigured` 追加
- テスト 3 件（empty/未設定 OK・不一致 WARNING・一致 OK + trim）

#### F-2. QUEUED レコードの poll 生存失効（レビュー 4-4）

- `RemoteLockRecord.lastPolledAt` 追加。`GET /acquire/{lockId}` が
  `RemoteLockManager.touchPoll()` 経由で更新（poll = 生存シグナル）
- `maybeScanStale()` に QUEUED 分岐追加: poll 途絶がしきい値
  （= STALE しきい値 60 秒、システムプロパティ `queuePollExpiryMs` で上書き可）超過で
  `FAILED` (`QUEUE_EXPIRED`) + `unqueueRemote()`
- 失効と昇格は `syncResources` で直列化（同時発生の競合なし）
- client 無改修: 失効後 poll は FAILED+QUEUE_EXPIRED か 404 で既存規則どおり終了
- テスト 2 件（途絶で失効・失効済みエントリがリソースを掴まない / poll 継続で生存・昇格）

#### F-3. 専用 RemoteUse 権限（レビュー 5-1）

- `LockableResourcesRootAction.REMOTE`（表示名 RemoteUse）を既存 PermissionGroup に追加
  （ADMINISTER に implied）
- `RemoteApiV1Action` の全 4 エンドポイントの `checkPermission(Jenkins.READ)` を
  `checkPermission(REMOTE)` に差し替え
- 運用: remote クライアントの credentialsId に使うマシンユーザーへ明示付与 →
  権限表で remote クライアントを監査可能に
- テスト 1 件（READ のみ → 全エンドポイント AccessDeniedException、
  REMOTE 付与ユーザー → acquire 202）

#### 完了条件

- 3 件とも実装 + テスト + 1 件 1 コミット
- `mvn test` (stabilize-build.sh) 全件 BUILD SUCCESS
- E2E 全 16 シナリオ回帰 PASS（既存環境は admin トークン = ADMINISTER implied のため無改修で通るはず）

- [x] 実装完了（コミット: `0ddc184` F-1 / `b79a669` F-2 / `02fcfae` F-3）
- [x] `mvn test` 確認完了（**360 件 / 0 失敗 / BUILD SUCCESS**、+6 件:
...
- [x] E2E 回帰確認完了（**全 16 シナリオ 16/16 PASS**、`--clean-start`、
  `dev/reports/20260612110631-e2e-test.md`。RemoteUse 権限変更込みの HPI で
  既存認証構成（admin トークン = ADMINISTER implied）が無改修で通ることを確認）

記録: 2026-06-12 実装完了。F-2 の失効しきい値はテスト用システムプロパティ
`queuePollExpiryMs` で短縮可能にした（既定 60 秒のまま）。
