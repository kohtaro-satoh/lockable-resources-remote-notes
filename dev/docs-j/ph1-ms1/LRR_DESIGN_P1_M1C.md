...
resolveRemoteAvailable(req) -> List<String> | null    // 現時点の空き解決
    ...
    label セレクタは quantity 個を貪欲取得（SEQUENTIAL）。
        **quantity 未指定/0 はマッチ全部**＝local "0 = all"（M1C follow-up）
...
- **quantity 未指定/0 = マッチ全部**（local "0 = all" 等価。M1C follow-up）。M1A 以降 remote は
  1 個に倒しており等価でなかった。`quantity > 総数` は充足不能のまま QUEUED（local 同様）。
...
| label quantity 未指定 = 全部【F-1】 | `quantity<=0` でマッチ全部をロック（local "0 = all" 等価）。POST 既定も 1→0 |
...
- 2026-06-12（M1C follow-up F-1）: label の quantity 未指定/0 を「マッチ全部」に修正
  （local "0 = all" 等価）。M1A 以降 remote は 1 個に倒していた長年の非等価。
  §3 と §7、E2E S15、ユニットテストで実証。
