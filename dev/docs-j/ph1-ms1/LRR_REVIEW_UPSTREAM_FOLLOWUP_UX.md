...
**本プロジェクトへの関与は Phase 1（#1055）＋ Phase 2 ＋ Phase 3 の計 3 PR で完了する予定**
（2026-07-26 方針確定）。残件は独立 PR を立てず、**Phase 2 PR に相乗りさせる**。

#### Phase 2 PR で一緒に直す

| 項目 | 出典 | 内容 |
|---|---|---|
| **`RemoteLockSession` の 404/410 ラベルのねじれ** | [§12.2](#122-本-pr-が表面化させた当方側の設計課題別サイクル送り) | 到達可能な `!bodyStarted` 側に timeout ラベル、到達不能な側に restart ラベルが付いている。当方（M1I）由来 |
| **doc `remote-api-curl.md` の状態表** | [§12-8](#12-指摘事項) | `EXPIRED` の誤記（サーバは返さない）・`STALE` の欠落・`heartbeatIntervalSeconds` がサーバ側で無視される点の未記載 |
| **Queue タブの Change Position ボタン** | [§12-3/4](#12-指摘事項) | remote 行にも描画され、押すと必ず 423。Phase 2 でクライアント UI を触る際に同時に潰す |
| **`X_SERVER_ID` / `X_LOCK_ID` の位置づけ明文化** | [§12.1](#121-設計思想透過等価との整合性チェック) | 「ブリッジ固有メタデータは透過等価の対象外」と設計書に記録するか、落とすかを決める |

#### 対応しない（意図的に見送り）

| 項目 | 理由 |
|---|---|
| **`.gitattributes`（`*.java text eol=lf`）の追加** | 環境依存の CRLF 再発を根本的に止められるが、**リポジトリ横断の規約変更**であり、Phase 1–3 で離脱する立場からは越権に見える。必要性は氏（maintainer）の判断に委ねる |
| **help URL 表記の統一**（[§6.2](#62-help-リンク-descriptor--descriptorbyname)） | 同上。`/descriptor/` 5 箇所と `/descriptorByName/` 3 箇所の二分は体裁の問題にとどまり、動作差はない（実機で両形式 200 を確認済み） |

> **タイミングの注意**: Change Position の 423 と doc の `EXPIRED` 誤記は、**#1055 が master に入った時点で
> リリース済みコードに載る**。Phase 2 までの間は利用者が踏み得る。氏が #1055 マージ前に自分で直す可能性も
> あるので、**マージ前に一報だけ入れておく**と手戻りが少ない（当方が抱える必要はない）。
