...
- runId: 20260719115008
- preset: stress
- plugin under test: `e83534b`
...
- `LOCK_WAIT_TIMEOUT` (clean allocate timeout, fail-closed): 18
...
- count: 1110  p50: 12458.5  p95: 168521.85  p99: 177529.1  max: 180548

## Resource utilization (docker stats)

| container | peak CPU% | peak mem (MiB) | net rx (MB) | net tx (MB) | samples |
|---|---|---|---|---|---|
| lrr-jenkins-a | 568.2 | 859.2 | 8.24 | 8.91 | 131 |
| lrr-jenkins-b | 574.5 | 844.7 | 7.57 | 7.82 | 131 |
| lrr-jenkins-c | 534.8 | 777.4 | 7.74 | 7.85 | 131 |
| lrr-jenkins-d | 680.0 | 759.7 | 6.53 | 6.56 | 131 |
...
![queue-wait-scatter](20260719115008-load-test/grid-storm/plots/queue-wait-scatter.png)

One point per lock acquisition: how long it waited (y) vs when it was acquired (x).

### Queued waiters over time

![queue-waiters-over-time](20260719115008-load-test/grid-storm/plots/queue-waiters-over-time.png)

Lock requests waiting (REQUESTed but not yet ACQUIRED) at each instant. Peaks = contention.

### Per-resource mean hold time

![resource-mean-hold-scatter](20260719115008-load-test/grid-storm/plots/resource-mean-hold-scatter.png)
...
![network-throughput](20260719115008-load-test/grid-storm/plots/network-throughput.png)

Per-container rx+tx throughput over time = load on the remote-lock REST API.

### Container CPU utilization

![cpu-utilization](20260719115008-load-test/grid-storm/plots/cpu-utilization.png)

Per-container CPU% over time; busier remote targets spike higher.

## Artifacts

- events: `20260719115008-load-test/grid-storm/events.csv`
- classification: `20260719115008-load-test/grid-storm/job-classification.csv`
- overlaps: `20260719115008-load-test/grid-storm/overlaps.txt`
- metrics: `20260719115008-load-test/grid-storm/metrics.json`

> Per-job console logs are omitted from this public bundle to keep it lean; the aggregated
> events/classification/metrics above capture the run.
