...
- `LOCK_WAIT_TIMEOUT` (clean allocate timeout, fail-closed): 10
...
- resource state changes recorded by the servers: 3836 (a=1012, b=974, c=934, d=916)
...
- count: 1160  p50: 2430.5  p95: 171669.99999999988  p99: 180474.82  max: 180569

## Resource utilization (docker stats)

| container | peak CPU% | peak mem (MiB) | net rx (MB) | net tx (MB) | samples |
|---|---|---|---|---|---|
| lrr-jenkins-a | 726.7 | 1034.2 | 8.27 | 8.89 | 136 |
| lrr-jenkins-b | 254.1 | 921.4 | 7.59 | 7.87 | 136 |
| lrr-jenkins-c | 255.8 | 879.2 | 7.07 | 7.05 | 136 |
| lrr-jenkins-d | 675.1 | 902.7 | 8.1 | 8.23 | 136 |
...
![queue-wait-scatter](20260814122345-load-test-stress/grid-storm/plots/queue-wait-scatter.png)

One point per lock acquisition: how long it waited (y) vs when it was acquired (x).

### Queued waiters over time

![queue-waiters-over-time](20260814122345-load-test-stress/grid-storm/plots/queue-waiters-over-time.png)

Lock requests waiting (REQUESTed but not yet ACQUIRED) at each instant. Peaks = contention.

### Per-resource mean hold time

![resource-mean-hold-scatter](20260814122345-load-test-stress/grid-storm/plots/resource-mean-hold-scatter.png)
...
![network-throughput](20260814122345-load-test-stress/grid-storm/plots/network-throughput.png)

Per-container rx+tx throughput over time = load on the remote-lock REST API.

### Container CPU utilization

![cpu-utilization](20260814122345-load-test-stress/grid-storm/plots/cpu-utilization.png)

Per-container CPU% over time; busier remote targets spike higher.

## Artifacts

- events: `20260814122345-load-test-stress/grid-storm/events.csv`
- classification: `20260814122345-load-test-stress/grid-storm/job-classification.csv`
- overlaps: `20260814122345-load-test-stress/grid-storm/overlaps.txt`
- metrics: `20260814122345-load-test-stress/grid-storm/metrics.json`
- consoles: `20260814122345-load-test-stress/grid-storm/consoles/`
