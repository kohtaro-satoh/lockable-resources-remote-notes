...
- runId: 20260805091309
- preset: stress
- plugin under test: `011c6a3`
...
- `LOCK_WAIT_TIMEOUT` (clean allocate timeout, fail-closed): 7
...
- count: 1168  p50: 4149.0  p95: 168855.59999999992  p99: 181065.32  max: 181154

## Resource utilization (docker stats)

| container | peak CPU% | peak mem (MiB) | net rx (MB) | net tx (MB) | samples |
|---|---|---|---|---|---|
| lrr-jenkins-a | 340.8 | 1118.2 | 8.21 | 8.89 | 125 |
| lrr-jenkins-b | 430.9 | 802.6 | 7.87 | 8.09 | 125 |
| lrr-jenkins-c | 224.4 | 1051.6 | 7.09 | 7.09 | 125 |
| lrr-jenkins-d | 258.5 | 1056.8 | 7.67 | 7.83 | 125 |
...
![queue-wait-scatter](20260805091309-load-test/grid-storm/plots/queue-wait-scatter.png)

One point per lock acquisition: how long it waited (y) vs when it was acquired (x).

### Queued waiters over time

![queue-waiters-over-time](20260805091309-load-test/grid-storm/plots/queue-waiters-over-time.png)

Lock requests waiting (REQUESTed but not yet ACQUIRED) at each instant. Peaks = contention.

### Per-resource mean hold time

![resource-mean-hold-scatter](20260805091309-load-test/grid-storm/plots/resource-mean-hold-scatter.png)
...
![network-throughput](20260805091309-load-test/grid-storm/plots/network-throughput.png)

Per-container rx+tx throughput over time = load on the remote-lock REST API.

### Container CPU utilization

![cpu-utilization](20260805091309-load-test/grid-storm/plots/cpu-utilization.png)

Per-container CPU% over time; busier remote targets spike higher.

## Artifacts

- events: `20260805091309-load-test/grid-storm/events.csv`
- classification: `20260805091309-load-test/grid-storm/job-classification.csv`
- overlaps: `20260805091309-load-test/grid-storm/overlaps.txt`
- metrics: `20260805091309-load-test/grid-storm/metrics.json`
- consoles: `20260805091309-load-test/grid-storm/consoles/`
