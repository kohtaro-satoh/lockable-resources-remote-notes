...
- runId: 20260814123643
- preset: timeout-race
- plugin under test: `111a767`
- harness (notes): `95f1e4b`
...
- `LOCK_WAIT_TIMEOUT` (clean allocate timeout, fail-closed): 108
...
- resource state changes recorded by the servers: 2134 (a=522, b=538, c=536, d=538)
...
- count: 634  p50: 793.0  p95: 60242.05  p99: 60294.67  max: 60397

## Resource utilization (docker stats)

| container | peak CPU% | peak mem (MiB) | net rx (MB) | net tx (MB) | samples |
|---|---|---|---|---|---|
| lrr-jenkins-a | 653.8 | 859.6 | 2.89 | 3.25 | 77 |
| lrr-jenkins-b | 283.6 | 847.9 | 2.9 | 3.09 | 77 |
| lrr-jenkins-c | 330.4 | 926.7 | 2.86 | 3.01 | 77 |
| lrr-jenkins-d | 670.3 | 946.4 | 2.84 | 2.99 | 77 |
...
![queue-wait-scatter](20260814123643-load-test-timeout-race/grid-storm/plots/queue-wait-scatter.png)

One point per lock acquisition: how long it waited (y) vs when it was acquired (x).

### Queued waiters over time

![queue-waiters-over-time](20260814123643-load-test-timeout-race/grid-storm/plots/queue-waiters-over-time.png)

Lock requests waiting (REQUESTed but not yet ACQUIRED) at each instant. Peaks = contention.

### Per-resource mean hold time

![resource-mean-hold-scatter](20260814123643-load-test-timeout-race/grid-storm/plots/resource-mean-hold-scatter.png)
...
![network-throughput](20260814123643-load-test-timeout-race/grid-storm/plots/network-throughput.png)

Per-container rx+tx throughput over time = load on the remote-lock REST API.

### Container CPU utilization

![cpu-utilization](20260814123643-load-test-timeout-race/grid-storm/plots/cpu-utilization.png)

Per-container CPU% over time; busier remote targets spike higher.

## Artifacts

- events: `20260814123643-load-test-timeout-race/grid-storm/events.csv`
- classification: `20260814123643-load-test-timeout-race/grid-storm/job-classification.csv`
- overlaps: `20260814123643-load-test-timeout-race/grid-storm/overlaps.txt`
- metrics: `20260814123643-load-test-timeout-race/grid-storm/metrics.json`
- consoles: `20260814123643-load-test-timeout-race/grid-storm/consoles/`
