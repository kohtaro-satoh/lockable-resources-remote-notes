...
- resource state changes recorded by the servers: 3864 (a=932, b=934, c=1004, d=994)
...
- count: 1168  p50: 8863.0  p95: 171498.6  p99: 177551.27  max: 180577

## Resource utilization (docker stats)

| container | peak CPU% | peak mem (MiB) | net rx (MB) | net tx (MB) | samples |
|---|---|---|---|---|---|
| lrr-jenkins-a | 647.3 | 988.5 | 7.96 | 8.6 | 135 |
| lrr-jenkins-b | 298.3 | 935.3 | 7.2 | 7.26 | 135 |
| lrr-jenkins-c | 338.9 | 883.4 | 8.59 | 8.81 | 135 |
| lrr-jenkins-d | 453.2 | 860.9 | 8.58 | 8.72 | 135 |
...
![queue-wait-scatter](20260813210938-load-test-stress/grid-storm/plots/queue-wait-scatter.png)

One point per lock acquisition: how long it waited (y) vs when it was acquired (x).

### Queued waiters over time

![queue-waiters-over-time](20260813210938-load-test-stress/grid-storm/plots/queue-waiters-over-time.png)

Lock requests waiting (REQUESTed but not yet ACQUIRED) at each instant. Peaks = contention.

### Per-resource mean hold time

![resource-mean-hold-scatter](20260813210938-load-test-stress/grid-storm/plots/resource-mean-hold-scatter.png)
...
![network-throughput](20260813210938-load-test-stress/grid-storm/plots/network-throughput.png)

Per-container rx+tx throughput over time = load on the remote-lock REST API.

### Container CPU utilization

![cpu-utilization](20260813210938-load-test-stress/grid-storm/plots/cpu-utilization.png)

Per-container CPU% over time; busier remote targets spike higher.

## Artifacts

- events: `20260813210938-load-test-stress/grid-storm/events.csv`
- classification: `20260813210938-load-test-stress/grid-storm/job-classification.csv`
- overlaps: `20260813210938-load-test-stress/grid-storm/overlaps.txt`
- metrics: `20260813210938-load-test-stress/grid-storm/metrics.json`
- consoles: `20260813210938-load-test-stress/grid-storm/consoles/`
