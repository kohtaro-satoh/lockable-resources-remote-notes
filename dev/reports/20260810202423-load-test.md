...
- runId: 20260810202423
- preset: stress
- plugin under test: `aa0c391`
...
- `LOCK_WAIT_TIMEOUT` (clean allocate timeout, fail-closed): 17
...
- count: 1142  p50: 9138.5  p95: 165523.85  p99: 174601.0  max: 180606

## Resource utilization (docker stats)

| container | peak CPU% | peak mem (MiB) | net rx (MB) | net tx (MB) | samples |
|---|---|---|---|---|---|
| lrr-jenkins-a | 448.7 | 922.3 | 8.48 | 9.12 | 132 |
| lrr-jenkins-b | 282.3 | 932.9 | 7.79 | 7.92 | 132 |
| lrr-jenkins-c | 537.6 | 996.7 | 8.21 | 8.35 | 132 |
| lrr-jenkins-d | 669.9 | 1008.0 | 7.49 | 7.59 | 132 |
...
![queue-wait-scatter](20260810202423-load-test/grid-storm/plots/queue-wait-scatter.png)

One point per lock acquisition: how long it waited (y) vs when it was acquired (x).

### Queued waiters over time

![queue-waiters-over-time](20260810202423-load-test/grid-storm/plots/queue-waiters-over-time.png)

Lock requests waiting (REQUESTed but not yet ACQUIRED) at each instant. Peaks = contention.

### Per-resource mean hold time

![resource-mean-hold-scatter](20260810202423-load-test/grid-storm/plots/resource-mean-hold-scatter.png)
...
![network-throughput](20260810202423-load-test/grid-storm/plots/network-throughput.png)

Per-container rx+tx throughput over time = load on the remote-lock REST API.

### Container CPU utilization

![cpu-utilization](20260810202423-load-test/grid-storm/plots/cpu-utilization.png)

Per-container CPU% over time; busier remote targets spike higher.

## Artifacts

- events: `20260810202423-load-test/grid-storm/events.csv`
- classification: `20260810202423-load-test/grid-storm/job-classification.csv`
- overlaps: `20260810202423-load-test/grid-storm/overlaps.txt`
- metrics: `20260810202423-load-test/grid-storm/metrics.json`
- consoles: `20260810202423-load-test/grid-storm/consoles/`
