...
- runId: 20260808142517
- preset: stress
- plugin under test: `ed2f5a9`
...
- `LOCK_WAIT_TIMEOUT` (clean allocate timeout, fail-closed): 24
...
- count: 1102  p50: 4128.5  p95: 138240.85000000012  p99: 174531.52  max: 180491

## Resource utilization (docker stats)

| container | peak CPU% | peak mem (MiB) | net rx (MB) | net tx (MB) | samples |
|---|---|---|---|---|---|
| lrr-jenkins-a | 306.7 | 1022.0 | 6.7 | 7.26 | 122 |
| lrr-jenkins-b | 401.6 | 842.4 | 7.77 | 8.06 | 122 |
| lrr-jenkins-c | 199.6 | 1092.6 | 7.18 | 7.32 | 122 |
| lrr-jenkins-d | 425.9 | 875.8 | 7.26 | 7.3 | 122 |
...
![queue-wait-scatter](20260808142517-load-test/grid-storm/plots/queue-wait-scatter.png)

One point per lock acquisition: how long it waited (y) vs when it was acquired (x).

### Queued waiters over time

![queue-waiters-over-time](20260808142517-load-test/grid-storm/plots/queue-waiters-over-time.png)

Lock requests waiting (REQUESTed but not yet ACQUIRED) at each instant. Peaks = contention.

### Per-resource mean hold time

![resource-mean-hold-scatter](20260808142517-load-test/grid-storm/plots/resource-mean-hold-scatter.png)
...
![network-throughput](20260808142517-load-test/grid-storm/plots/network-throughput.png)

Per-container rx+tx throughput over time = load on the remote-lock REST API.

### Container CPU utilization

![cpu-utilization](20260808142517-load-test/grid-storm/plots/cpu-utilization.png)

Per-container CPU% over time; busier remote targets spike higher.

## Artifacts

- events: `20260808142517-load-test/grid-storm/events.csv`
- classification: `20260808142517-load-test/grid-storm/job-classification.csv`
- overlaps: `20260808142517-load-test/grid-storm/overlaps.txt`
- metrics: `20260808142517-load-test/grid-storm/metrics.json`
- consoles: `20260808142517-load-test/grid-storm/consoles/`
